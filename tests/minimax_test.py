...
from src.engine.constants import DEFAULT_DEPTH

from src.engine.evaluators.eval import Eval
from src.engine.evaluators.mock_eval import MockEval  # Updated import
from src.engine.search.minimax import Minimax
from src.engine.search.zobrist import Zobrist


DEFAULT_TEST_CONFIG = EngineConfig()
...
class TestMoveOrdering:
    ...
    def test_order_moves_returns_list(self):
        ...
        minimax = Minimax(board, evaluator, DEFAULT_TEST_CONFIG)
...
class TestMinimaxWithZobrist:
    ...
    def test_minimax_correctness_with_zobrist(self, fen):
        ...
        # Create configs with and without Zobrist

        config_no_zobrist = EngineConfig(
            search_depth=6, minimax=MinimaxConfig(use_zobrist=False)
        )
        config_with_zobrist = EngineConfig(
            search_depth=6, minimax=MinimaxConfig(use_zobrist=True)
        )

        minimax_no_zobrist = Minimax(board.copy(), evaluator, config_no_zobrist)
        score_no_zobrist, move_no_zobrist = minimax_no_zobrist.find_top_move(depth=6)

        minimax_with_zobrist = Minimax(board.copy(), evaluator, config_with_zobrist)
