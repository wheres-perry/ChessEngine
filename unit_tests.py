...
class TestConfigValidation:
    def test_default_config_valid(self):
        ...
        config = EngineConfig()
        assert config.minimax.use_alpha_beta is True
        assert config.minimax.use_zobrist is True

    def test_lmr_without_alpha_beta_raises(self):
        with pytest.raises(
            ValueError,
            match=r"Late Move.*alpha.*beta",
        ):
            EngineConfig(
    ...
    def test_lmr_without_move_ordering_raises(self):
        with pytest.raises(
            ValueError,
            match=r"Late Move.*move.*ordering",
        ):
            EngineConfig(
                minimax=MinimaxConfig(
                    use_alpha_beta=True,
    ...
    def test_tt_aging_without_zobrist_raises(self):
        with pytest.raises(
            ValueError,
            match=r"Transposition.*table.*Zobrist",
        ):
            EngineConfig(
                minimax=MinimaxConfig(
                    use_zobrist=False,
    ...
    def test_invalid_search_depth_raises(self):
        with pytest.raises(ValueError, match=r"Search depth"):
            EngineConfig(search_depth=0)
        with pytest.raises(ValueError, match=r"Search depth"):
            EngineConfig(search_depth=-1)


class TestLMRBasics:
    def test_lmr_enabled_by_default(self):
        """Test that LMR is enabled by default in the configuration."""
        cfg = EngineConfig()
        assert cfg.minimax.use_lmr is True

    def test_lmr_can_be_disabled(self):
        """Test that LMR can be disabled."""
        cfg = EngineConfig(minimax=MinimaxConfig(use_lmr=False))
        assert cfg.minimax.use_lmr is False

    def test_lmr_integration_basic(self):
        """Test basic LMR integration without checking internal constants."""
        cfg = EngineConfig()
        board = chess.Board()
        engine = Minimax(board, MockEval(board), cfg)

        # Just verify the engine can be created with LMR enabled
        # (no engine.config, just check cfg)

        assert engine.use_lmr is True

        # Test that search can run (basic integration test)

        try:
            # This should not raise an exception

            score, move = engine.find_top_move()
            assert move is not None
        except Exception as e:
            pytest.fail(f"LMR-enabled search failed: {e}")
