...
[[nodiscard]] constexpr inline uint8_t orient_square(bool is_white_pov,
                                                     uint8_t square) noexcept {
...
get_piece_index(PieceType pt, Color piece_color, bool is_white_pov) noexcept {
...
halfkp_index(bool is_white_pov, uint8_t king_square, uint8_t piece_square,
             PieceType pt, Color piece_color) noexcept {
...
board_to_halfkp_indices(const Board &board, bool is_white_pov) noexcept;

// Convert board to dense float32 tensor (both perspectives concatenated)
// Output: array of size TOTAL_FEATURES
[[nodiscard]] std::vector<float>
board_to_input_tensor(const Board &board) noexcept;
