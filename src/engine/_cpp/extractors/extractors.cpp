...
std::vector<int> extract_halfkp(const Board &board) {
  ...
  if (!king_sq_opt)
    return active_indices; // Safe fallback
  ...
  while (all_pieces) {
    ...
    if (auto p = board.piece_at(sq)) {
      if (p->type == PieceType::KING)
        continue; // HalfKP excludes kings from piece features

      // Piece mapping: 0-4 for STM pieces, 5-9 for enemy pieces
      int is_enemy = (p->color == stm) ? 0 : 5;
      int pt_idx = static_cast<int>(
          p->type); // PAWN=0, KNIGHT=1, BISHOP=2, ROOK=3, QUEEN=4
...
GraphData extract_gnn(const Board &board) {
  ...
  while (temp) {
    ...
    case PieceType::PAWN:
      ...
      // Note: Pawns attack diagonally. Straight pushes are not "edges" in
      // standard GNN.
      break;
