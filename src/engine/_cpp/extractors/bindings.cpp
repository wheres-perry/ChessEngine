...
void init_extractors_bindings(py::module_ &m) {
    py::module_ ext_m = m.def_submodule("extractors", "Machine learning feature extractors");

    ext_m.def("extract_cnn", [](const Board& board) -> py::array_t<float> {
    ...
    ext_m.def("extract_halfkp", [](const Board& board) -> py::array_t<int> {
    ...
    ext_m.def("extract_gnn", [](const Board& board) -> py::dict {
        ...
        std::copy(data.nodes.begin(), data.nodes.end(), static_cast<int*>(buf_nodes.ptr));
        ...
        return result;
    }, py::arg("board"), "Extract a PyTorch Geometric compatible Graph representation");
}