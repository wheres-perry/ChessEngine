    ...
    while (std::getline(file_, line)) {
        ...
        if (line.empty()) {
            if (reading_moves && !move_text.empty()) {
                ...
                // in some loose PGN formats, but standard PGN expects the result marker.
                // We'll keep reading to be safe.
            }
            continue;
        }
        ...
        if (line[0] == '[') {
            if (reading_moves) {
                // Encountered new headers while reading moves (e.g., missing empty line).
                // We need to seek back so the next iteration processes this line.
                // But std::getline doesn't seek back cleanly without offsets.
                // For a highly robust parser we'd handle this, but standard PGN files 
